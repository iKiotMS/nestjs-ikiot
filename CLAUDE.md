...
**Not yet wired up, in rough priority order for porting business logic from iKiotMS-BE** (per the "Thứ tự build đề xuất" in the Feature Ledger artifact — remaining groups: Order/Promotion/CashDrawer, Ticket/Stats/AI, then the deferred HR set — PayrollSetting/Paysheet/ShiftTemplate/Holiday and Schedule/Attendance/LeaveRequest, which the owner asked to leave for later):
